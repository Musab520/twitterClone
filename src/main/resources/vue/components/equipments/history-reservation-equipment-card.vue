<template id="history-reservation-equipment-card">
  <div>
    <v-card
        class="mx-auto"
        outlined>
      ...
      >
        <v-chip rounded readonly class="  float-right ma-2" :color="getavailabilityColor(equipment_availability)"
                style="
    font-weight: normal;
    font-size: 12px;"
                dark>
...
<!--        <v-icon color="accent">mdi-file-document-outline</v-icon>-->
    ...
    <v-row class="d-flex justify-start justify-space-between  py-3 mt-0 ml-4 mr-3">
      <div>
      <span class="text-subtitle" style="font-size: 11px" >{{ $trans('historyPage.equipmentCardDetailsForReservation.dateFrom') }}</span><br/>
      <span color="black"  style="font-size: 15px;font-weight: 400">  {{reservation_datefrom}}</span>
      </div>
      <div>
        <span class="text-subtitle" style="font-size: 11px" >{{ $trans('historyPage.equipmentCardDetailsForReservation.dateTo') }}</span><br/>
      ...
      <v-card-actions>
          <v-btn v-if="equipment_status=='Disabled'"
              ...
              large
                 disabled
              color="primary">
            {{ $trans('historyPage.equipmentCardDetailsForReservation.currentlyUnavailableButton') }}
          </v-btn>
          <v-btn v-else
              outlined
              block
              large
              color="primary">
            {{ $trans('historyPage.equipmentCardDetailsForReservation.reserveAgainButton') }}
          </v-btn>
...
<script>
Vue.component("history-reservation-equipment-card", {
  template: "#history-reservation-equipment-card",
  props: {
    id: {
