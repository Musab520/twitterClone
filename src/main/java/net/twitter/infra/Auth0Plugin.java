...
public class Auth0Plugin implements Plugin {
    private static final Logger log = LoggerFactory.getLogger(Auth0Plugin.class);
    ...
    private void handleLoginRedirect(Context ctx) {
    ...
    private void handleAuth0Callback(Context ctx) {
        ...
        try {
            ...
            log.warn(String.format("Auth0 Identity Verification Error: %s", e.getMessage()));
            ...
            log.warn(String.format("Failed to handle auth0 callback : %s", e.getMessage()));
        }
    }
